...
WatsonDiscoverySetup.prototype.getDiscoveryConfig = function (params) {
  return new Promise((resolve, reject) => {
    this.discoveryClient.listConfigurations(params, (err, data) => {
      if (err) {
        if (err.code === 501) {
          // this feature is disabled in newer versions of Disco, which is running on CPD
  ...
  return new Promise((resolve, reject) => {
    this.discoveryClient.listEnvironments(params, (err, data) => {
      if (err) {
        ...
        console.log('environments[0]: ' + JSON.stringify(environments[0], null, 2));
        ...
        params.name = DISCOVERY_ENVIRONMENT_NAME;
        return resolve(params);
      }
    });
  });
  ...
  return new Promise((resolve, reject) => {
    this.discoveryClient.listCollections(params, (err, data) => {
      if (err) {
        ...
        console.log('no collection found - must create');
...
WatsonDiscoverySetup.prototype.loadDiscoveryCollection = function (params) {
  ...
  this.discoveryClient.getCollection(params, (err, data) => {
    if (err) {
      console.log('ERROR during: Get collection to check its status.');
      console.error(err);
    } else {
      ...
      const loadedDocs = result.document_counts.available + result.document_counts.processing + result.document_counts.failed;
